...
func main() {
	err := (&App{
		...
		Flags: []Flag{
			...
			&BoolFlag{Name: "project", Aliases: []string{"p"}, Usage: "init as ts project Eg: engine -dp"},
		...
		Action: func(c *Context) error {
			...
			if c.Bool("define") {
				if c.Bool("project") {
					engine.DumpDefines("./typings")
					return os.WriteFile("tsconfig.json", []byte(`{
  "compilerOptions": {
    "typeRoots": [
      "./typings"
    ],
...
func executeStdIn(c *Context, ts bool, warm time.Duration, timeout time.Duration, withTimeout bool) error {
	...
	cc := fn.WithSignal(func(ctx context.Context) {
		...
		if withTimeout {
			...
			defer cc()
			if dbg {
				c, m := engine.CompileSourceWithMapping("vm.xs", src, ts, true)
				vm.Debug = true
				vm.SourceMap = m
				v = fn.Panic1(vm.RunCodeContext(c, warm, cx))
			} else {
